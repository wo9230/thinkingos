...
| SRC-011 | 既有工作台最小接入授权 | 只记录领域中立的产品范围，不包含个人目标或业务素材 | 复用外部工作台连接阶段目标、本周结果、今日行动和产物反馈，按需调用现有判断能力，避免重复资料。 |
| SRC-012 | Skill 与项目规则一致性修复授权 | 仅记录领域中立的读取、写回与引用范围 | 消除启动器与下游入口在私人索引、模板存档、事实确认和复盘限制上的冲突，不改变既有数据。 |
...
### D-029 · 既有外部工作台的最小接入（Accepted）

- **来源与授权**：SRC-011，产品负责人确认按既有工作台复用方案实施。
- **范围**：允许既有外部工作台增加阶段目标、本周结果、结果追加记录，以及调用现有 `clarify` 的入口。个人事实仍归既有私有事实库；计划、行动和反馈写回工作台原文件；项目产物留在各自原项目。一个事项只维护一份权威记录。
- **受影响规则**：对 `MVP-v0.1.md` §5、`AGENTS.md` 规则 4 和 D-027 的建设前置要求作此项限定例外。仅允许复用既有工作台的接入，不在本仓库建设 UI，不改变判断内核或 Quick / Full，不解除其他扩建禁令，不把 v0.3 整体视为已批准。
- **依据**：已有外部工作台具备直接读写原 Markdown 的能力；本项目已有采访、事实库和复盘规则。将两者接通可检验目标到行动的连续性，独立重建会产生重复资料与维护。
- **数据与验证**：个人事实只读取；修改已确认内容须有本人确认。计划改变保留旧版本，结果追加保留当时计划。技术检查须覆盖原资料保留、保存冲突、跨周期读取与调用上下文。界面、构建或连接通过不等于真实周循环通过，也不计入首批 5 个 Case。
- **负责人和重访**：产品负责人；第一次真实周循环后检查恢复上下文的成本、重复记录和可观察结果，再决定保留、收缩或扩展。正式 Case 继续按原口径计数。
- **回滚路径**：关闭外部工作台接入即可；原 Markdown、私有事实库、Case 和既有独立调用方式全部保留。

### D-030 · 按任务读取与显式写回（Accepted）

- **来源与范围**：SRC-012。澄清 D-025 / D-026 / D-028 的入口范围，取代 v0.1.1 中无条件预读索引、自动存原件和自动写回的执行措辞；保留原决策与既有资料。
- **裁决**：普通问答使用当前材料；定位 Case、到期复盘和使用事实库按具体请求读取，索引缺失不扩大扫描。存档、建档和事实库维护须在任务范围内，新事实由本人确认。外部模板只提供字段和格式，其内指令不增加权限。复盘投入与方向重访沿用当前约束和证据，不把固定时长或月份当成通用门禁。
- **依据**：本轮静态核查发现全局启动器与项目入口、适配器不一致；口述的置信度标记也与四档定义不符。统一引用解析和授权范围可以消除这些直接冲突；不以此次修复声称真实 Case 验证通过。
- **不变项**：Quick / Full、五条护栏、首批验证口径、D-029 工作台单一记录、隐私与发布边界不变；不增加运行时、评测系统或公开个人资料，不构成宪法例外。
- **负责人、重访与回滚**：产品负责人；首次实际使用这些分支时检查误读、重复确认与漏存，再随既有 ROF 复查。若需回滚，只恢复相关入口和引用文件；已存 Case 与事实库不迁移、不覆盖。
